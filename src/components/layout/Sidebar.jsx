...
import { LayoutDashboard, Bed, Calendar, Users, UserCog, ChevronLeft, ChevronRight, LogOut, User } from 'lucide-react';
...
export function Sidebar() {
    ...
    return (<aside className={cn("fixed left-0 top-0 h-screen bg-sidebar text-sidebar-foreground transition-all duration-300 z-50 flex flex-col", collapsed ? "w-20" : "w-64")}>
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-6 space-y-1">
        {navigation.map((item) => {
            ...
            return (<Link key={item.name} to={item.href} className={cn("flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 group", isActive
              ...
              {!collapsed && (<span className="font-medium animate-fade-in">{item.name}</span>)}
            </Link>);
        })}
      </nav>
      ...
      <div className="px-3 py-4 border-t border-sidebar-border space-y-3">
        {user && !collapsed && (<div className="flex items-center gap-3 px-3 py-2 animate-fade-in">
        ...
        <Button variant="ghost" onClick={handleLogout} className={cn("w-full justify-start gap-3 text-sidebar-foreground/80 hover:bg-destructive/10 hover:text-destructive", collapsed && "justify-center px-0")}>
          <LogOut className="h-5 w-5"/>
          {!collapsed && <span>Logout</span>}
        </Button>

        <button onClick={() => setCollapsed(!collapsed)} className="flex w-full items-center justify-center gap-2 px-3 py-2 rounded-lg text-sidebar-foreground/60 hover:bg-sidebar-accent hover:text-sidebar-foreground transition-colors">
