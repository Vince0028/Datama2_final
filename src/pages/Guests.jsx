...
export default function Guests() {
    ...
    const uniqueGuests = Array.from(new Map(reservations
        .flatMap(r => r.guests)
        .filter(g => g?.guest)
        .map(g => [g.guest.Guest_ID, g.guest])).values());
    ...
    const getGuestHistory = (guestId) => {
        return reservations.filter(r => r.guests.some(g => g.guest.Guest_ID === guestId));
    };
    ...
    return (<div className="animate-fade-in">
      <PageHeader title="Guest Directory" description="View and search all registered guests"/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredGuests.map((guest, index) => {
            ...
            return (<Dialog key={guest.Guest_ID}>
              <DialogTrigger asChild>
                <div className="bg-card rounded-xl shadow-card p-6 cursor-pointer transition-all duration-300 hover-lift animate-slide-up" style={{ animationDelay: `${index * 50}ms` }} onClick={() => setSelectedGuestId(guest.Guest_ID)}>
              ...
              <DialogContent className="sm:max-w-[600px]">
                ...
                <div className="py-4">
                  {/* Guest Details */}
                  {selectedGuest && (<div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    {guestHistory.length > 0 ? (<div className="space-y-3">
                        {guestHistory.map((reservation) => (<div key={reservation.Reservation_ID} className="flex items-center justify-between p-4 rounded-lg bg-muted/50">
                            <div>
                              ...
                              {reservation.payment?.Method && (<p className="text-xs text-muted-foreground mt-1">
                      ...
                      </div>) : (<p className="text-muted-foreground text-center py-4">No reservation history</p>)}
                  </div>
                </div>
              </DialogContent>
            </Dialog>);
