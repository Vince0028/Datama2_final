...
const getRoomImage = (type, roomId) => {
...
export default function GuestRooms() {
    ...
    return (<div className="container mx-auto py-8 animate-fade-in">
            <PageHeader title="Our Rooms" description="Find your perfect sanctuary."/>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-border mt-6 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <input type="date" className="w-full p-2 border rounded-md" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} min={new Date().toISOString().split('T')[0]}/>
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-medium">Check-Out Date</label>
                        <input type="date" className="w-full p-2 border rounded-md" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} min={checkIn || new Date().toISOString().split('T')[0]}/>
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayRooms.map((room) => (<Card key={room.Room_ID} className="overflow-hidden group h-full flex flex-col">
                        <div className="h-48 overflow-hidden relative">
                            <img src={room.image} alt={room.typeName} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>
                        ...
                        <CardFooter>
                            {room.isBookable ? (<div className="w-full space-y-2">
                                    <RoomScheduleDialog roomId={room.Room_ID} roomName={`Room ${room.Room_Number}`}/>
                                    <GuestBookingDialog roomName={`Room ${room.Room_Number} - ${room.typeName}`} price={room.price} roomID={room.Room_ID} initialCheckIn={checkIn} initialCheckOut={checkOut}/>
            ...
            {displayRooms.length === 0 && (<div className="text-center py-12">
