...
const formatId = (id, digits = 3) => String(id).padStart(digits, '0');
export default function Reservations() {
    ...
    const filteredReservations = reservations.filter(res => {
        const matchesSearch = (res.room?.Room_Number?.includes(searchTerm) ||
        ...
        if (activeTab === "active")
            return matchesSearch && (res.Status === "Booked" || res.Status === "CheckedIn");
    ...
    return (<div className="animate-fade-in space-y-6">
      <PageHeader title="Reservations" description="Manage bookings and approvals" action={<StaffBookingDialog />}/>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <Tabs defaultValue="all" className="w-full md:w-[400px]" onValueChange={setActiveTab}>
          <TabsList>
            ...
            <TabsTrigger value="pending" className="flex items-center gap-2">
              Pending
              {reservations.filter(r => r.Status === 'Pending').length > 0 && (<span className="bg-yellow-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <Button variant="outline" size="icon" onClick={handleRefresh} disabled={isRefreshing} title="Refresh reservations">
            <RefreshCw className={`h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`}/>
          </Button>
          <div className="relative flex-1 md:flex-none">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
            <Input placeholder="Search guests or rooms..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 w-full md:w-[300px]"/>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-card shadow-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            return (<TableRow key={reservation.Reservation_ID} className="animate-fade-in">
                  ...
                  <TableCell>
                    {reservation.staff ? (<span className="text-sm">{reservation.staff.First_Name} {reservation.staff.Last_Name}</span>) : (<span className="text-xs text-yellow-600 bg-yellow-100 px-2 py-0.5 rounded-full flex items-center w-fit gap-1">
                  ...
                  <TableCell className="text-right">
                    {reservation.Status === 'Pending' ? (<div className="flex justify-end gap-2">
