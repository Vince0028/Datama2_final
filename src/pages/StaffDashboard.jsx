...
export default function StaffDashboard() {
    ...
    return (<div className="animate-fade-in">
            <div className="flex items-center justify-between mb-8">
                <PageHeader title="Staff Dashboard" description="Hotel Operations & Revenue Overview"/>

                {/* Reset Data Button - Managers Only */}
                {user?.staffData?.Role === 'Manager' && (<button onClick={() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <StatCard title="Total Revenue" value={formatCurrency(metrics.totalRevenue)} icon={<DollarSign className="h-6 w-6"/>} variant="primary" trend={{ value: 12, isPositive: true }}/>
                <StatCard title="Active Reservations" value={metrics.activeReservations} subtitle="Coming/In-house" icon={<Calendar className="h-6 w-6"/>}/>
                <StatCard title="Available Rooms" value={metrics.availableRooms} subtitle="Ready for booking" icon={<Bed className="h-6 w-6"/>}/>
                <StatCard title="Average Stay" value={`${metrics.averageStay.toFixed(1)} nights`} icon={<Clock className="h-6 w-6"/>} variant="accent"/>
            </div>
            ...
            return (<div key={reservation.Reservation_ID} className="flex items-center justify-between p-4 rounded-lg bg-muted/50 hover:bg-muted transition-colors animate-slide-up" style={{ animationDelay: `${index * 50}ms` }}>
            ...
            return (<div key={item.method} className="animate-slide-up" style={{ animationDelay: `${index * 100}ms` }}>
                                    ...
                                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                                        <div className="h-full gradient-accent rounded-full transition-all duration-500" style={{ width: `${percentage}%` }}/>
                                    </div>
