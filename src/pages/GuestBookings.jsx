...
export default function GuestBookings() {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            ...
            try {
                ...
                const reservationIds = resGuests.map((rg) => rg.reservation_id);
                // Get reservations
                ...
                // Join the data
                const enrichedBookings = (reservations || []).map((res) => {
                    const room = rooms?.find((r) => r.room_id === res.room_id);
                    const roomType = room ? roomTypes?.find((rt) => rt.roomtype_id === room.roomtype_id) : null;
                    const payment = paymentsData?.find((p) => p.reservation_id === res.reservation_id);
                    return {
                        ...res,
    ...
    return (<div className="container py-8">
      ...
      {bookings.length === 0 ? (<Card>
          ...
          {bookings.map((booking) => (<Card key={booking.reservation_id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <div className="grid sm:grid-cols-4 gap-4 text-sm">
                  ...
                  {booking.payment_method && (<div className="flex items-center gap-2">
                      <CreditCard className="h-4 w-4 text-muted-foreground"/>
                      <div>
