...
export default function Rooms() {
    ...
    const filteredRooms = enrichedRooms.filter(room => {
        if (statusFilter !== 'All' && room.Status !== statusFilter)
            return false;
        if (typeFilter !== 'All' && room.roomType.Type_Name !== typeFilter)
            return false;
        return true;
    });
    ...
    const roomHasActiveBooking = (roomId) => {
        return reservations.some(r => r.Room_ID === roomId && ['Pending', 'Booked', 'CheckedIn'].includes(r.Status));
    };
    // Check if a room is actively occupied today (check-in <= today < check-out)
    const roomIsOccupiedToday = (roomId) => {
        const today = new Date().toISOString().split('T')[0];
        return reservations.some(r => r.Room_ID === roomId
            && ['Booked', 'CheckedIn'].includes(r.Status)
            && r.Check_In <= today
            && r.Check_Out > today);
    };
    const handleStatusChange = (roomId, value) => {
    ...
    return (<div className="animate-fade-in">
      <PageHeader title="Room Administration" description="Manage hotel inventory and status" action={<Button variant="outline" size="sm" onClick={handleRefresh} disabled={isRefreshing}>
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        <div className="flex items-center gap-2">
          ...
          {['All', 'Available', 'Occupied', 'Maintenance'].map((status) => (<Button key={status} variant={statusFilter === status ? 'default' : 'outline'} size="sm" onClick={() => setStatusFilter(status)}>
              {status}
            </Button>))}
        </div>
        <div className="flex items-center gap-2">
          ...
          {Array.from(new Set(rooms.map(r => r.roomType?.Type_Name).filter(Boolean))).map((typeName) => (<Button key={typeName} variant={typeFilter === typeName ? 'default' : 'outline'} size="sm" onClick={() => setTypeFilter(typeName)}>
              {typeName}
            </Button>))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredRooms.map((room, index) => (<div key={room.Room_ID} className={cn("relative rounded-xl border-2 p-6 transition-all duration-300 hover-lift animate-slide-up", statusColors[room.Status])} style={{ animationDelay: `${index * 50}ms` }}>
            ...
            <div className="pt-4 border-t border-border/50">
              ...
              <Select value={room.Status} onValueChange={(value) => handleStatusChange(room.Room_ID, value)} disabled={isReadOnly}>
