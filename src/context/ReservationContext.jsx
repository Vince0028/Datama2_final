...
function mapRoom(r, roomTypesData) {
    const rTypeId = r.roomtype_id || r.RoomType_ID;
    const matchType = roomTypesData?.find((rt) => (rt.roomtype_id || rt.RoomType_ID) === rTypeId);
    return {
...
async function fetchRoomsFromDB() {
    ...
    if (typesRes.error)
        throw new Error('Room types: ' + typesRes.error.message);
    if (roomsRes.error)
        throw new Error('Rooms: ' + roomsRes.error.message);
    const types = typesRes.data || [];
    const mapped = (roomsRes.data || []).map((r) => mapRoom(r, types));
...
async function fetchReservationsFromDB() {
    ...
    return (resRes.data || []).map((r) => {
        const room = roomsArr.find((rm) => rm.room_id === r.room_id);
        const roomType = room ? typesArr.find((rt) => rt.roomtype_id === room.roomtype_id) : null;
        const resGuests = rgArr
            .filter((rg) => rg.reservation_id === r.reservation_id)
            .map((rg) => {
            const guest = guestsArr.find((g) => g.guest_id === rg.guest_id);
            return {
                ResGuest_ID: rg.resguest_id,
                Reservation_ID: rg.reservation_id,
                Guest_ID: rg.guest_id,
        ...
        const staff = r.staff_id ? staffArr.find((s) => s.staff_id === r.staff_id) : null;
        // Find payment for this reservation
        const payment = paymentsArr.find((p) => p.reservation_id === r.reservation_id);
        return {
            Reservation_ID: r.reservation_id,
            Room_ID: r.room_id,
            Staff_ID: r.staff_id || 0,
...
export function ReservationProvider({ children }) {
    ...
    useEffect(() => {
        ...
        // Realtime updates
        const roomCh = supabase.channel('rooms-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'room' }, (p) => {
            if (p.eventType === 'UPDATE') {
                setRooms(prev => prev.map(r => r.Room_ID === (p.new.room_id || p.new.Room_ID)
                    ? { ...r, Status: p.new.status || p.new.Status }
                    : r));
            }
        }).subscribe();
        const resCh = supabase.channel('reservations-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'reservation' }, (p) => {
            if (p.eventType === 'UPDATE') {
                setReservations(prev => prev.map(res => res.Reservation_ID === p.new.reservation_id
        ...
        const runAutoCheckout = async () => {
            ...
            setReservations(prev => {
                const expired = prev.filter(r => {
                    if (r.Status !== 'Booked' && r.Status !== 'CheckedIn')
                        return false;
                ...
                setRooms(prevRooms => prevRooms.map(room => expiredRoomIds.has(room.Room_ID) ? { ...room, Status: 'Available' } : room));
                return prev.map(r => expiredIds.has(r.Reservation_ID) ? { ...r, Status: 'CheckedOut' } : r);
            });
        };
    ...
    const parseConstraintError = (msg) => {
    ...
    const updateStatus = async (id, requestedStatus) => {
        let status = requestedStatus;
        const targetRes = reservations.find(r => r.Reservation_ID === id);
        const previousStatus = targetRes?.Status;
        // Optimistic update
        setReservations(prev => prev.map(res => res.Reservation_ID === id
    ...
    const updateRoomStatus = async (id, status) => {
        // Optimistic update
        setRooms(prev => prev.map(room => room.Room_ID === id ? { ...room, Status: status } : room));
        try {
    ...
    const checkAvailability = (roomId, checkIn, checkOut) => {
        ...
        const conflict = reservations.some(res => {
            if (res.Room_ID !== roomId)
                return false;
            if (res.Status === 'Cancelled' || res.Status === 'CheckedOut')
                return false;
    ...
    const paymentBreakdown = Object.entries(paymentByMethod).map(([method, amount]) => ({
        method,
        amount
    }));
    return (<ReservationContext.Provider value={{ reservations, rooms, addReservation, addWalkInReservation, updateStatus, metrics, paymentBreakdown, resetData, updateRoomStatus, checkAvailability, refreshData }}>
            {children}
